// BJJRats — WhatsApp do Superadmin
// Permite conectar o WhatsApp da plataforma para disparos automáticos
...
export default function AdminWhatsapp() {
  ...
  // Polling durante tentativa de conexão
  useEffect(() => {
    if (!polling) return;
  ...
  }, [polling, expireAttempt]);

  // Contagem regressiva
  useEffect(() => {
    if (!polling || !(qrcode || qrCodeText)) return;
    const timer = setInterval(() => {
      setAttemptRemainingMs(prev => Math.max(0, prev - 1000));
    }, 1000);
    return () => clearInterval(timer);
  }, [polling, qrcode, qrCodeText]);

  // Expira ao zerar
  useEffect(() => {
    if (attemptRemainingMs <= 0 && (qrcode || qrCodeText)) expireAttempt();
  }, [attemptRemainingMs, qrcode, qrCodeText, expireAttempt]);
  ...
  return (
    <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div>
        <p style={{ fontFamily: FONTS.condensed, fontWeight: 900, fontSize: '1.1rem', textTransform: 'uppercase', color: '#FFF', margin: 0 }}>📱 WHATSAPP DA PLATAFORMA</p>
        <p style={{ fontFamily: 'Barlow, sans-serif', fontSize: '0.75rem', color: '#555', marginTop: '0.25rem' }}>
          Conecte o WhatsApp oficial da plataforma para enviar notificações e anúncios
        </p>
      </div>
      ...
      {status?.connected ? (
        <div style={{ background: '#0A1A0A', border: '1px solid #1A4A1A', borderLeft: '3px solid #25D366', padding: '1.25rem' }}>
          ...
          <div style={{ marginTop: '1rem', padding: '0.75rem', background: '#111', border: '1px solid #222' }}>
            <p style={{ fontFamily: 'Barlow, sans-serif', fontSize: '0.75rem', color: '#888', margin: 0, lineHeight: 1.5 }}>
              Mensagens de anúncios e notificações da plataforma serão enviadas por este número.
            </p>
          </div>
        </div>
      ) : attemptExpired ? (
        ...
        <div style={{ background: '#111', border: '1px solid #222', padding: '1.5rem', textAlign: 'center' }}>
          <p style={{ fontFamily: FONTS.condensed, fontWeight: 900, fontSize: '1rem', color: '#888', textTransform: 'uppercase', margin: 0, marginBottom: '1rem' }}>WHATSAPP NÃO CONECTADO</p>
          <p style={{ fontFamily: 'Barlow, sans-serif', fontSize: '0.75rem', color: '#555', marginBottom: '1.25rem', lineHeight: 1.5 }}>
            Conecte o WhatsApp da plataforma para enviar anúncios e notificações automáticas.
          </p>
          <button onClick={handleConnect} disabled={connecting}
            style={{ background: '#25D366', border: 'none', color: '#111', fontFamily: FONTS.condensed, fontWeight: 900, fontSize: '0.8rem', textTransform: 'uppercase', padding: '0.75rem 1.25rem', cursor: connecting ? 'not-allowed' : 'pointer', opacity: connecting ? 0.65 : 1, width: '100%' }}>
